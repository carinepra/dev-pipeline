...
Compatível com v2.1 (fallback para defaults White/PROJ).
"""
...
_V2_FALLBACK_PROJECTS = {
    "PROJ": {
        "name": "White PROJ",
        "repos": ["white-backend", "white-frontend", "docs"],
        "component_repo_map": {
            "Front": "white-frontend",
            "Back": "white-backend",
            "Infra": "white-backend",
            "Docs": "docs",
        },
        "docs_search_paths": [
            "white-docs-tech/app/docs/content/sistemas/white/"
        ],
    }
}
...
_V2_FALLBACK_REPOSITORIES = {
    "white-backend": {
        "github_repo": "white-api",
        "local_path": "white-api",
        "type": "backend",
        "tech_stack": "python/fastapi",
    },
    "white-frontend": {
        "github_repo": "white-app",
        "local_path": "white-app",
        "type": "frontend",
        "tech_stack": "nextjs/typescript",
    },
    "docs": {
        "github_repo": "white-docs-tech",
        "local_path": "white-docs-tech",
        "type": "docs",
        "tech_stack": "markdown",
    },
}
...
def get_pipeline_config() -> dict:
    ...
    if cfg.get("version", "2.1") < "3.0":
        cfg.setdefault("jira", {
            "base_url": os.getenv("JIRA_SERVER", "https://whitecompany.atlassian.net"),
            "default_project": "PROJ",
            "projects": _V2_FALLBACK_PROJECTS,
        })
        cfg.setdefault("github", {"org": "WhiteCompany"})
...
def is_issue_key(value: str) -> bool:
    """Retorna True se valor é uma issue key válida (ex: PROJ-542, INVEST-100)."""
...
def get_jira_base_url() -> str:
    ...
    return cfg.get("jira", {}).get(
        "base_url",
        os.getenv("JIRA_SERVER", "https://whitecompany.atlassian.net"),
    )
...
def get_default_project() -> str:
    ...
    cfg = get_pipeline_config()
    return cfg.get("jira", {}).get("default_project", "PROJ")


def get_github_org() -> str:
    """Retorna GitHub org do config."""
    cfg = get_pipeline_config()
    return cfg.get("github", {}).get("org", "WhiteCompany")
...
def resolve_component_to_repo(project_key: str, component: str | None) -> dict:
    ...
    cfg = get_pipeline_config()
    org = cfg.get("github", {}).get("org", "WhiteCompany")
    repo["org"] = org
