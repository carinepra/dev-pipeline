...
1. **Criar o plano da história** — consolidar Figma, docs e codebase no `.story-plan.md`.
2. **Incentivar discussão técnica** — Steps 4 e 5 existem para **alinhar** contexto, conflitos e decisões arquiteturais; convide o time a questionar até não restar ambiguidade relevante.
3. **Rodar a skill de revisão de plano** — em seguida, **obrigatoriamente** invocar `pipeline-plan-validator` (Step 7) sobre o mesmo `.story-plan.md`, para endurecer o plano em 3 rounds.
4. **Só depois** da revisão: o pipeline/orquestrador pode perguntar se pode **executar** a próxima etapa (ex.: quebrar em tasks). **Sempre confirmar explicitamente** com o usuário antes de seguir — **nunca** assumir consentimento silencioso.
...
if scope in ["backend", "frontend", "both"]:
    ...
    if config_path.exists():
        ...
        pass  # implementação real atribui search_paths a partir do config
...
### 1.4 Contexto do Banco de Dados (OPCIONAL — backend/both)

**Execute SOMENTE SE:**
1. O escopo for `backend` ou `both`
2. A descrição da história mencionar persistência de dados (BD, tabela, entidade, schema)
3. Pelo menos um repo backend tiver `database` configurado no `.pipeline-config.json`

**Se qualquer condição não for atendida, pule para o Step 2.**

### Como descobrir o BD dos repos afetados
...
# Coletar db_config de todos os repos backend do projeto
backend_repos = [
    (repo_id, repo_cfg)
    for repo_id, repo_cfg in config["repositories"].items()
    if repo_cfg.get("type") == "backend" and repo_id in project_config.get("repos", [])
]

db_contexts = {}

for repo_id, repo_cfg in backend_repos:
    db_config = repo_cfg.get("database")
    if not db_config:
        continue

    print(f"🗄️ [{repo_id}] BD: {db_config['type']} — {db_config.get('description', '')}")
    ...
    if db_config["type"] == "dynamodb":
        ...
        if db is None:
            print(f"   ⚠️ Indisponível (credenciais ausentes ou tabela inacessível)")
            db_contexts[repo_id] = {"type": "dynamodb", "available": False}
            continue

        # Inferir entities relevantes para esta história
        # Ex.: história de pedidos → ["ORDER"]; RH → ["EMPLOYEE"]
        target_entities = ["<inferir da descrição da história>"]

        repo_db_context = {"type": "dynamodb", "table": db.table_name, "entities": {}}
        for entity in target_entities:
            items = db.query_by_gsi("HIERARCHY", "entity", entity, limit=2)
            if items:
                repo_db_context["entities"][entity] = {
                    "real_attributes": list(items[0].keys()),
                    "available_gsis": ["HIERARCHY", "STATUS", "TIMESTAMP", "GSI1", "GSI2"],
                }
                print(f"   ✅ {entity}: {list(items[0].keys())}")

        db_contexts[repo_id] = repo_db_context

    else:
        # Tipo reconhecido mas sem client implementado
        print(f"   ⚠️ Tipo '{db_config['type']}' sem client implementado")
        print(f"   {db_config.get('notes', '')}")
        db_contexts[repo_id] = {"type": db_config["type"], "note": db_config.get("notes", "")}
```

**Use `db_contexts` no `.story-plan.md`** para informar o task breaker:
- Schema real das entities envolvidas
- Índices/GSIs disponíveis para as queries previstas
- Alertas de repos cujo BD não tem suporte implementado ainda
...
**Antes de qualquer “próxima etapa” automática:** o orquestrador deve obter **confirmação explícita** de que o plano da história está aceito e que pode seguir (ex.: task breakdown). Não executar etapas seguintes só porque o arquivo foi salvo.
